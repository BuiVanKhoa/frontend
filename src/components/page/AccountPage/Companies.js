...
class Companies extends React.Component {
  ...
  componentDidMount(){ //khi component dc load thi su kien se dong thoi xay ra
        this.props.fetchListCompaniesAccount();
      
  }

  render() {
    const listCompanies= this.props.listCompaniesAccount
    return (
      <>
        ...
        <div className="list_acc">
          <table className="table">
            ...
            <tbody>
              <Table col="7" dt={listCompanies} accountType="companies"></Table>
            </tbody>
          </table>
        </div>
      </>
...
const mapStateToProps =(state)=>{ //lay state trog store
  return {
    listCompaniesAccount: state.crudCompanies
  }
}
const mapDispatchToProps =(dispatch)=>{ // gui action tuong ung vs su kien
  return{
    fetchListCompaniesAccount: () => dispatch ( accountAction.fetchListCompaniesAccount())
  }
}
